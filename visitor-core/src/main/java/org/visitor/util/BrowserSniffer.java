...
/**
 * Sniffer business value from UA.
 * @author mengw
 *
 */
public class BrowserSniffer implements java.io.Serializable {

	private static final long serialVersionUID = 5259550015527181123L;
	...
	private String getFirstVersionNumber(String a_userAgent, int a_position,
			int numDigits) {
	...
	private String[] getBrowser(String userAgent) {
		...
		if ((pos = userAgent.indexOf("Lotus-Notes/")) > -1) {
			...
			res = getArray("Opera",
					...
					"Opera" + getVersionNumber(userAgent, pos + 5));
			...
			res = getArray("KHTML", "Chrome", "KHTML(Chrome"
					+ getVersionNumber(userAgent, pos + 6) + ")");
		} else if ((pos = userAgent.indexOf("Safari/")) > -1) {
			res = getArray("KHTML", "Safari", "KHTML(Safari"
					+ getVersionNumber(userAgent, pos + 6) + ")");
		} else if ((pos = userAgent.indexOf("Konqueror/")) > -1) {
			res = getArray("KHTML", "Konqueror", "KHTML(Konqueror"
					+ getVersionNumber(userAgent, pos + 9) + ")");
	...
	String[] platforms = { "Windows", "PalmOS", "Symbian", "Windows CE", "osx",
			"Linux", "iphone", "" };

	String[] browsers = { "Opera", "MSIE", "Chrome", "Firefox", "Nintendo",
			"Nitro", "OPWV", "UP.Browser", "UP.Link", "Nokia Mobile Browser",
			"Nokia", "Maemo", "AvantGo", "DoCoMo", "KDDI", "EudoraWeb",
			"iPhone", "iPAQ", "PPC", "portalmmm", "Safari", "Tablet browser",
			"O2", "SPV", "Blazer", "EPOC", "amaya", "AmigaVoyager",
			"Amiga-AWeb", "bluefish", "BrowseX", "Camino", "Charon", "Chimera",
			"curl", "Democracy", "Dillo", "Obigo", "KBrowser", "ccWAP-Browser",
			"EzWAP", "jBrowser", "safari", "Mozilla" };
	String[] mobiles = { "SonyEricsson", "Nokia", "Android", "BlackBerry",
			"SymbianOS", "AvantGo", "DoCoMo", "iPhone", "iPAQ", "mobile",
			"ccWAP-Browser", "EzWAP", };

	String[] bots = { "Yahoo", "Baidu", "Googlebot", "Google", "WebCrawler",
			"Inktomi", "Teoma", "MSNBot" };
	...
	public String getBrowserVersion() {
		...
		getBrowserName();
		if (StringUtils.isNotBlank(_versionNumber)) {
			name = _versionNumber;
		}
		return name;
	}
	
	public String getOperaBrowserFullVersion() {
		String name = "unknown";
		try {
			String test = agent.toLowerCase();
			if (test.indexOf("opera mini/") > -1) {
				String[] s = test.split("opera mini/", 2);
				s = s[1].split(" ", 2);
				s = s[0].split("/");
	...
	public String getOperaBrowserVersion() {
		String name = "unknown";
		try {
			String test = agent.toLowerCase();
			if (test.indexOf("opera mini/") > -1) {
	...
	public boolean isMobile() {
		if (agent != null) {
			...
			for (String x : mobiles) {
				if (test.indexOf(x.toLowerCase()) > -1)
					return true;
			}
		}
		return false;
	...
	public static void main(String[] args) {
		String ua = "Opera/9.80 (Android; Opera Mini/6.1.2589/6.1; U; zh) Presto/2.5.25 Version/10.54";
		BrowserSniffer bs = new BrowserSniffer(ua);
		System.err.println(bs.getOperaBrowserFullVersion());
	}
}
