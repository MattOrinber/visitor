 ...
 * 
 */
@Component("DailyUpdateProductExcelView")
public class DailyUpdateProductExcelView extends AbstractExcelView {
	private static final SimpleDateFormat fileSdf = new SimpleDateFormat("yyyyMMdd");
	private String fileName = "ProductDailyUpdate" + fileSdf.format(new Date()) + ".xls";

	/**
	 * 
	 */
	public DailyUpdateProductExcelView() {
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		header.createCell(9).setCellValue("更新人");
		header.createCell(10).setCellValue("更新时间");
		header.createCell(11).setCellValue("版本");
		...
		for (Iterator<ProductList> it = list.iterator(); it.hasNext();) {
			...
			row.createCell(7).setCellValue(product.getPublishDate());
			row.createCell(8).setCellValue(product.getCreateBy());
			row.createCell(9).setCellValue(product.getModBy());
			row.createCell(10).setCellValue(sdf.format(product.getModDate()));
			row.createCell(11).setCellValue(product.getProductVersion());
		}
	}
	...
	private String getProductFolder(List<Folder> folders) {
		...
		if (null != folders) {
			for (Folder f : folders) {
				sb.append(f.getFolderId()).append("  ")
				  .append(f.getName())
				  .append("\n");
			}
		}
