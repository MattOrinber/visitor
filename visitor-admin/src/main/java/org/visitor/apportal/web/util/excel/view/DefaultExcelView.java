package org.visitor.apportal.web.util.excel.view;
...
import org.visitor.apportal.web.util.excel.annotation.ExcelField;
...
public class DefaultExcelView extends AbstractExcelView {
    ...
    protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook,
        ...
        if (testList.size() > 0) {
        	sampleObject = testList.get(0);
		}

        //excel header
        buildExcelHead(filename, cls, sampleObject, workbook);
     ...
     */
    private void buildExcelHead(String filename, Class<?> cls, Object sampleObject, HSSFWorkbook workbook) throws SecurityException, NoSuchMethodException {
        // Initialize
        List<String> headerList = Lists.newArrayList();
        // check result list is or not in header list
        for (Object[] os : annotationList) {
            String t = ((ExcelField) os[0]).title();
            
            Object val = null;
            String property = ((ExcelField) os[0]).value();
            if (StringUtils.isNotBlank(property)) {
            	 val = Reflections.invokeGetter(sampleObject, property);
			}else{				
				if (os[1] instanceof Field) {
					val = Reflections.invokeGetter(sampleObject, ((Field) os[1]).getName());
				}else if (os[1] instanceof Method) {
					val = Reflections.invokeMethod(sampleObject, ((Method) os[1]).getName(),
							new Class[] {}, new Object[] {});
				}
			}
